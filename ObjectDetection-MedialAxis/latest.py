...
def OneVideoOutLine(file):
    ...
    while(True):
        ...
        if ret == True:
            ...
            frame_black = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            fgmask = fgbg.apply(frame_black)
...
def OneVideoOutLineSeg(file):
    ...
    while(True):
        ...
        if ret == True:
            ...
            rhoAvg = 0
            count = 1
            thetaAvg = theatAvg_old
            rhoAvg = rhoAvg_old
            if hlines is not None:
                mid = (int)((len(hlines))/2) 
                thetaAvg = hlines[mid][0][1]
                rhoAvg = hlines[mid][0][0]
                theatAvg_old = thetaAvg
                rhoAvg_old = rhoAvg
                for hline in hlines:
                    # rhoAvg = hline[0][0]
                    # thetaAvg = hline[0][1]
